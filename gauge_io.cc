...
int read_gf_ildg(su3 gf[], char* filename){
  ...
  if(bytes == (uint64_t)LX*LY*LZ*T*4*(18*sizeof(float)) && ( (precision == 32) || (readprecision == 0) )) { /* file contains floats  3*3*2 floats *4 wg 4 Richtungen*/

  
  bytes = sizeof(su3)/2; /* halbe Größe, wegen floats */
  ...
  if(bytes == (uint64_t)LX*LY*LZ*T*4*(18*sizeof(double)) && ( (precision == 64) || (readprecision == 0) )) { /* file contains doubles  3*3*2 doubles *4 wegen 4 Richtungen */
  bytes = (uint64_t) sizeof(su3);  
...
/*  xlf- info */
  ...
  if (prec == 64){
    bytes = 4*T*LX*LY*LZ*su3numofbytes;
  }
  else if(prec == 32){
    bytes = 4*T*LX*LY*LZ*su3numofbytes/2;
  }
  ...
  if(prec == 64){ 
    counter = 0;
    bytes =  su3numofbytes;  /* number of bytes in spin[x] */
    ...
    if(prec == 32){
      bytes =  su3numofbytes/2;  /* number of bytes in spin[x] */
...
int calculate_cksums_trafo(type_cksum cksums[],char* basefilename, su3 * trafofield){/* Calculate in BIG-endian!!! */
  int nt,nz,fieldpos;                                                                /* this is the routine for the*/
...
int write_conf_info(char* filename, type_conf_info confinfo, type_cksum cksums[]){
  ...
  fprintf(ofs, " Program bqcd YM-JSCC-1.2 [$Rev: 21 $ $Date: 2008-04-24 20:33:50 +0930 (Thu, 24 Apr 2008) $]\n");
  fprintf(ofs, " Run         999\n");
...
int read_conf_info(char* filename, type_conf_info confinfo){
  ...
  printf("Reading info file '%s'...\n",filename);
   ...
   fgets(line,100,ifs); /* Program bqcd YM-JSCC-1.2 [$Rev: 21 $ $Date: 2008-04-24 20:33:50 +0930 (Thu, 24 Apr 2008) $] */
